...
- Checkout starts through a server API route that creates a Shopify checkout and redirects to Shopify-hosted checkout.
...
- Centralized server-side checkout mutation in `src/lib/shopify.server.js`.
...
2. Move to `cartCreate` + `cartLinesUpdate` instead of legacy checkout mutation path.
